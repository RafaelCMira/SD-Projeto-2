...
EOF
    ...
    # Adiciona o Subject Alternative Name (SAN) ao certificado
    keytool -keystore "$nome_keystore" -alias "$servidor" -ext SAN=dns:"$servidor" -storepass "$password" -keypass "$password" -noprompt
done
